...
class SalesBot:
    def __init__(self):
        ...
        logger.info(f"Sales Bot: {Config.SALES_BOT_NAME}")
        ...
        try:
            ...
            async for message in websocket:
                try:
                    ...
                    # Prefer event metadata when Exotel provides explicit media format details
                    ...
                    if event_sample_rate is not None and event_sample_rate != detected_sample_rate:
                        logger.info(
                            f"Overriding sample rate from event metadata: "
    ...
    def _extract_sample_rate_from_websocket_path(self, websocket_path: str) -> int:
        ...
        try:
            ...
            for key in ("sample-rate", "sample_rate", "samplerate"):
                ...
                logger.warning(
                    f"Unsupported sample rate '{parsed_rate}' in query param '{key}'. "
                    f"Falling back to default {self.default_sample_rate}Hz"
                )
                return self.default_sample_rate
        except Exception as e:
            logger.warning(f"Failed to parse sample rate from websocket path: {e}")
    ...
    def _initialize_connection_settings(self, stream_id: str, sample_rate: int, start_data: dict):
        ...
        logger.info(f"Enhanced Events: {self.exotel_enhanced_events}")
    ...
    async def handle_exotel_media(self, stream_id: str, data: dict):
        ...
        if stream_id not in self.openai_connections:
            logger.warning(f"No OpenAI connection for {stream_id} - ESTABLISHING NOW")
            await self.connect_to_openai_enhanced(stream_id)
        ...
        if stream_id in self.openai_connections:
            ...
            logger.warning(f"Still no OpenAI connection for {stream_id} after connection attempt")
    ...
    async def handle_exotel_clear(self, stream_id: str, data: dict):
        ...
        if stream_id in self.openai_connections:
            ...
            logger.warning(f"No OpenAI connection to clear for {stream_id}")
    ...
    def _get_sales_instructions(self) -> str:
        """Get sales bot system instructions"""
        return f"""You are {Config.SALES_REP_NAME}, a warm, concise sales assistant for {Config.COMPANY_NAME}.

Your goals:
1. Greet the caller warmly and professionally
2. Understand their needs and interests
3. Present relevant products and pricing options
4. Answer questions about features and benefits
5. Schedule product demonstrations when appropriate
6. Offer to transfer to a specialist if needed

Always be:
- Professional and courteous
- Clear and concise in explanations
- Responsive to caller needs
- Ready to schedule follow-ups

If the caller asks for a specialist or has complex requirements, offer to transfer them to a human representative."""
    ...
    async def connect_to_openai_enhanced(self, stream_id: str):
        ...
        try:
            ...
            # Set sales-specific instructions
            session_config['instructions'] = self._get_sales_instructions()
        ...
        try:
            ...
            greeting_msg = {
                ...
                "item": {
                    ...
                    "content": [{
                        "type": "input_text", 
                        "text": f"A customer just called our sales line. \
                        The connection is running at {sample_rate}Hz audio quality. \
                ...
                try:
                    ...
                    if event_type in ("response.output_audio.delta", "response.audio.delta"):
                        ...
                        if transcript_delta.strip():
                            logger.info(f"SARAH SPEAKING: {transcript_delta}")
    ...
    async def handle_openai_audio_delta_enhanced(self, stream_id: str, data: dict):
        ...
        try:
            if stream_id not in self.exotel_connections:
                logger.warning(f"No Exotel connection for {stream_id}")
                return
    ...
    def _resample_audio(self, audio_data: bytes, from_rate: int, to_rate: int) -> bytes:
        ...
        try:
            ...
            if resampled:
                ...
                logger.warning(f"RESAMPLING FAILED, using original audio")
                return audio_data
...
async def main():
    """Enhanced main function to start the Sales Bot"""
    try:
